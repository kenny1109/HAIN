    ...
    for i in range(start, end+1):
        ...
        for frame in load_dict:
            ...
            for sq, turn in enumerate(frame['turns']):
                tmp.append(turn['utterance'])
                dial_len += len(turn['utterance'])
                if (sq + 1) % 2 == 0:
...
class DataProcess:
    ...
    @staticmethod
    def pad_sentence(doc, pad=0, max_length=600):
...
class LoadData(DataProcess):
    def __init__(self, path, max_length=500, vocab_size=50000):
        ...
        if load is False:
            ...
            if shuffle:
                ...
                self.val_x, self.test_x, self.val_y, self.test_y = train_test_split(self.test_x, self.test_y,
                                                                                    ...
                                                                                    random_state=rnd,
                                                                                    stratify=self.test_y)
            self.train_x = [re.sub(r"\n", " ", string) for string in self.train_x]
            self.val_x = [re.sub(r"\n", " ", string) for string in self.val_x]
            self.test_x = [re.sub(r"\n", " ", string) for string in self.test_x]
