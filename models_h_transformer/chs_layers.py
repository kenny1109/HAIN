...
class DMLC(nn.Module):
    def __init__(self, w2v_weight, vocab_size, embedding_dim, hidden_dim, batch_size, num_labels):
        ...
        self.dl = DecisionLayer(hidden_dim)
...
class EmbeddingLayer(nn.Module):
    def __init__(self, w2v_weight, vocab_size, embedding_dim, hidden_dim, batch_size, num_heads=4, head_dim=50):
        ...
        encoder_layer = nn.TransformerEncoderLayer(embedding_dim, 4, 128, 0.1, activation='gelu')
    ...
    def forward(self, sequence, mask):
        ...
        #     o.append(output)
        # #     attention_visualization(w.detach().numpy(), ax=axes[i])
        # # print("word level multi-head")
        # # plt.show()
...
class SentenceLayer(nn.Module):
    def __init__(self, input_dim, hidden_dim, batch_size, num_labels, num_heads=4, head_dim=50):
        ...
        encoder_layer = nn.TransformerEncoderLayer(input_dim, 4, 128, 0.1, activation='gelu')
    ...
    def forward(self, input, mask):
        ...
        #     o.append(output)
        # #     attention_visualization(w.detach().numpy(), ax=axes[i])
        # # print("sentence level multi-head")
        # # plt.show()
        # output = torch.cat(o, dim=-1)
        # output = self.linear(output)
        # output = self.drop(output)
...
class DecisionLayer(nn.Module):
    def __init__(self, input_dim, hidden_dim=100, num_class=19, n_head=4, dropout=0.1):
    ...
    def forward(self, input):
        output = self.transformer_encoder(input, mask=None)
        attn_output_weights = None
        # o, attn_output_weights = self.transformer_encoder.layers[0].self_attn(input, input, input, attn_mask=None)
        ...
        # attention_visualization(interaction.detach().squeeze().numpy(), min=-10, max=10)
        # plt.show()
        ...
        # attention_visualization(attn_output_weights.detach().squeeze().numpy())
        # print("label level multi-head")
        # plt.show()
