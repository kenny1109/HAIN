...
def load_word2vec(model_type, vocabulary_inv, num_features=300):
    ...
    for i, w in enumerate(vocabulary_inv):
        if w in embedding_model:
            ...
            # print(vocabulary_inv[w], w)
            if w == '<pad>':
                embedding_weights.append(np.random.uniform(0, 0, num_features))
            else:
                # embedding_weights.append(np.random.randn(num_features))
...
if __name__=='__main__':
    ...
    # path = '/data/dstc8.p'
    ld = LoadData(path)
    ld.load_data(load=False, save=False)
    print(ld.change_label_shape(ld.encode_train_y))
    em_w = load_word2vec('glove', ld.word2index, 50)
